...
func TestMemoryStoreTaskGitBackupAndSnapshotCRUD(t *testing.T) {
	...
	backups, err := s.TaskGitBackups(ctx, "task-1")
	if err != nil || len(backups) != 1 || backups[0].Paths[0] != "README.md" {
		t.Fatalf("TaskGitBackups = %+v, %v", backups, err)
	}

	snapshot := domain.TaskGitTurnSnapshot{ID: "snap-1", TaskID: "task-1", BeforeTree: "aaa", AfterTree: "bbb", CreatedAt: now, UpdatedAt: now}
	if err := s.SaveTaskGitTurnSnapshot(ctx, snapshot); err != nil {
		t.Fatalf("SaveTaskGitTurnSnapshot returned error: %v", err)
	}
	loaded, err := s.LatestTaskGitTurnSnapshot(ctx, "task-1")
	if err != nil || loaded.AfterTree != "bbb" {
		t.Fatalf("LatestTaskGitTurnSnapshot = %+v, %v", loaded, err)
	}
}
