...
func TestSQLStoreVersionedMigrationListsDeletionAndHelpers(t *testing.T) {
	...
	for _, table := range []string{"task_review_runs", "task_review_findings", "task_review_comments"} {
		hasTable, err := sqliteTableHasColumn(ctx, sqlStore, table, "id")
		if err != nil {
			t.Fatal(err)
		}
		if hasTable {
			t.Fatalf("%s should be absent after versioned migrations", table)
		}
	}
...
func TestSQLStoreInitialSchemaOmitsTaskReviewTables(t *testing.T) {
	ctx := context.Background()
	...
	for _, table := range []string{"task_review_runs", "task_review_findings", "task_review_comments"} {
		...
		if hasTable {
			t.Fatalf("%s should not be created by initial schema", table)
		}
	}
}
